...
class PersistenceManager:
    ...
    def _load_tasks(self) -> None:
        ...
        try:
            if self.tasks_file.exists():
                ...
                if isinstance(
                        data,
                        dict) and 'tasks' in data and 'version' in data:
                    self._tasks_cache = data['tasks']
                    logger.info(
                        f"Loaded {len(self._tasks_cache)} tasks from {self.tasks_file}")
                else:
                    logger.warning(
                        f"Invalid data structure in {
                            self.tasks_file}, starting fresh")
                    self._tasks_cache = {}
            else:
                logger.info(
                    f"Tasks file {
                        self.tasks_file} not found, starting fresh")
                self._tasks_cache = {}
    ...
    def _save_tasks(self, force: bool = False) -> None:
        ...
        if not force and (
                current_time -
                self._last_save_time) < self._save_interval:
            return
        ...
        try:
            ...
            logger.debug(
                f"Saved {len(self._tasks_cache)} tasks to {self.tasks_file}")
    ...
    def update_task_status(
            ...
            **kwargs) -> Optional[ProcessingTask]:
        """
        更新任务状态并记录历史
            ...
            if new_status == 'processing' and 'processing_started_at' not in task_data:
                ...
                if 'processing_started_at' in task_data and task_data['processing_started_at']:
                    try:
                        ...
                        logger.error(
                            f"Failed to calculate processing duration: {e}")
    ...
    def add_file_metadata(
        ...
        with self._lock:
            if task_id not in self._tasks_cache:
                logger.warning(
                    f"Task {task_id} not found for file metadata update")
                return
            ...
            self._save_tasks()
            logger.info(
                f"Added file metadata for task {task_id}: hash={file_hash[:8]}..., size={file_size}")
        ...
        with self._lock:
            ...
            for task_id, task_data in self._tasks_cache.items():
                try:
                    ...
                    logger.error(
                        f"Failed to parse task data for {task_id}: {e}")
                    continue
            return tasks
...
def init_persistence(data_dir: str = "data") -> PersistenceManager:
    ...
    if _persistence_manager is None:
        ...
        logger.info(
            f"Initialized persistence manager with data_dir: {data_dir}")
    else:
        logger.info(
            "Persistence manager already initialized, reusing existing instance")
    return _persistence_manager
