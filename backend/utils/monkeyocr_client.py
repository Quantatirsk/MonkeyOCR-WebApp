...
class MonkeyOCRClient:
    ...
    async def process_file(
                    ...
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            if "download_url" in result or "url" in result:
                                ...
                                # Handle case where response format is
                                # different
                                return {
                                    "success": True,
                                    "download_url": None,
                            ...
                            if attempt < self.max_retries - 1:
                                ...
                                raise MonkeyOCRAPIError(
                                    "Rate limited by MonkeyOCR API")

                        else:
                            error_msg = f"MonkeyOCR API error: {
                        ...
                        if attempt < self.max_retries - 1:
                            ...
                            raise MonkeyOCRNetworkError(
                                "MonkeyOCR API request timed out")
        ...
        raise MonkeyOCRAPIError(
            "Failed to process file after all retry attempts")
            ...
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                ...
                async with client.stream("GET", download_url) as response:
                    if response.status_code == 200:
                        ...
                        raise MonkeyOCRDownloadError(
                            f"Failed to download result: {response.status_code} - {response.text}")
    ...
    def _resolve_base_url(self) -> str:
        ...
        try:
            ...
            if ip_address:
                ...
                print(
                    f"Resolved MonkeyOCR base URL: {resolved_url} (from {hostname})")
                return resolved_url
            else:
                print(
                    f"Failed to resolve {hostname}, using fallback: {fallback_url}")
                return fallback_url

        except Exception as e:
            print(
                f"Error resolving base URL: {e}, using fallback: {fallback_url}")
            return fallback_url
    ...
    async def health_check(self, retry_with_refresh: bool = True) -> bool:
        ...
        try:
            async with httpx.AsyncClient(timeout=10) as client:
                ...
                if response.status_code < 500:
                    ...
                    print(
                        f"Health check failed with status {
                            response.status_code}")

        except Exception as e:
            print(f"Health check failed: {e}")

        # If initial health check failed and retry is enabled, try refreshing
        # URL
        if retry_with_refresh:
            ...
            if new_url != old_url:
                print(
                    f"Base URL changed from {old_url} to {new_url}, retrying health check...")
                # Prevent infinite recursion
                return await self.health_check(retry_with_refresh=False)
            else:
