...
async def sync_tasks(
    ...
    last_sync: Optional[str] = Query(
        ...
        task_ids: Optional[str] = Query(
            None,
            description="Comma-separated list of client task IDs"),
    ...
    try:
        ...
        client_task_ids = []
        if task_ids:
            client_task_ids = [tid.strip()
                               for tid in task_ids.split(',') if tid.strip()]
        ...
        if last_sync_timestamp and len(filtered_tasks) > 0:
            ...
            filtered_tasks = [
                task for task in filtered_tasks if (
                    hasattr(
                        task,
                        'last_modified') and task.last_modified and datetime.fromisoformat(
                        task.last_modified) > last_sync_timestamp) or (
                    hasattr(
                        task,
                        'created_at') and datetime.fromisoformat(
                            task.created_at) > last_sync_timestamp)]

        # Sort by creation order (most recent first for better UX)
        filtered_tasks.sort(
            key=lambda t: t.created_at if hasattr(
                t, 'created_at') else t.id, reverse=True)
        ...
        api_response = APIResponse(
            success=True,
            data=sync_response,
            message=f"Sync completed ({sync_type}): {
                len(filtered_tasks)} tasks",
            error=None)
...
async def get_original_file(task_id: str, response: Response):
    ...
    try:
        ...
        etag = hashlib.md5(
            f"{original_file_path}:{file_mtime}:{file_size}".encode()).hexdigest()
        ...
        response.headers["Last-Modified"] = datetime.fromtimestamp(
            file_mtime).strftime("%a, %d %b %Y %H:%M:%S GMT")
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get original file: {
                str(e)}")
...
async def get_task_preview(task_id: str):
    ...
    try:
        ...
        if preview_info["file_exists"]:
            ...
            if file_ext in ['.jpg', '.jpeg', '.png', '.gi', '.bmp', '.webp']:
                preview_info["file_type"] = "image"
            elif file_ext == '.pdf':
                preview_info["file_type"] = "pd"
            else:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get preview: {
                str(e)}")
