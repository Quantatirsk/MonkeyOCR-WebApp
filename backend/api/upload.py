...
async def upload_file(
    ...
    try:
        ...
        if file.content_type not in SUPPORTED_FILE_TYPES:
            raise HTTPException(
                ...
                detail=f"Unsupported file type: {
                    file.content_type}. Supported types: {
                    list(
        ...
        try:
            ...
            logger.warning(
                f"Failed to save original file for task {task_id}: {e}")
...
@router.get("/tasks/{task_id}/status",
            response_model=APIResponse[ProcessingTask])
    ...
    try:
        ...
        try:
            ...
            if result.get("download_url"):
                await monkeyocr_client.download_result(
                    result["download_url"],
                    task_id
                )
