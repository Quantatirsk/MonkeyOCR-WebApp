...
class ProcessingTask(BaseModel):
    ...
    file_hash: Optional[str] = Field(
        None, description="SHA256 hash of original file")
    file_size: Optional[int] = Field(
        None, description="Size of original file in bytes")
    total_pages: Optional[int] = Field(
        None, description="Total pages for PDF files")
    last_modified: Optional[str] = Field(
        None, description="Last modification timestamp")
    status_history: List[TaskStatusHistory] = Field(
...
class UploadRequest(BaseModel):
    ...
    extract_type: Literal['standard', 'split',
                          'text', 'formula', 'table'] = 'standard'
